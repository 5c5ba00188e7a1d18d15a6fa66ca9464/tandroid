...
public abstract class StoriesUtilities {
    ...
    */
    ...
    /* JADX WARN: Code restructure failed: missing block: B:78:0x0178, code lost:
        if (((org.telegram.tgnet.tl.TL_stories$StoryItem) r11.stories.get(r6)).close_friends != false) goto L61;
     */
        ...
        if (avatarStoryParams.drawHiddenStoriesAsSegments) {
            ...
            if (unreadState != 2) {
                ...
                if (i <= 1) {
                    ...
                    paint6.setAlpha(NotificationCenter.voipServiceCreated);
                    return;
                }
                float f2 = 360.0f / i;
                ...
                while (i3 < i) {
                    ...
                    if (avatarStoryParams.drawHiddenStoriesAsSegments) {
                        ...
                        if (i3 < tL_stories$PeerStories.stories.size()) {
                            if (((TL_stories$StoryItem) tL_stories$PeerStories.stories.get(i3)).justUploaded || ((TL_stories$StoryItem) tL_stories$PeerStories.stories.get(i3)).id > max) {
                            }
                            paint5 = paint9;
                        }
                        paint5 = paint2;
            ...
            arrayList = tL_stories$PeerStories.stories;
        }
        i = arrayList.size();
        if (unreadState != 2) {
        }
    ...
    public static EnsureStoryFileLoadedObject ensureStoryFileLoaded(TL_stories$PeerStories tL_stories$PeerStories, final Runnable runnable) {
        ...
        while (true) {
            if (i2 >= tL_stories$PeerStories.stories.size()) {
                ...
                break;
            } else if (((TL_stories$StoryItem) tL_stories$PeerStories.stories.get(i2)).id > i) {
