    ...
    private void set(TLRPC$TL_messages_stickerSet tLRPC$TL_messages_stickerSet) {
        ...
        while (true) {
            if (i >= arrayList.size()) {
                tLRPC$Document = null;
                break;
            } else if (((TLRPC$Document) arrayList.get(i)).id == tLRPC$TL_messages_stickerSet.set.thumb_document_id) {
                tLRPC$Document = (TLRPC$Document) arrayList.get(i);
                break;
            } else {
                i++;
    ...
    public void load(final int i, boolean z, TLObject tLObject, final ArrayList arrayList, final Object obj) {
        ...
        if (!(tLObject instanceof TLRPC$Photo)) {
            ...
            if (lastRequestParentObject == obj || (tLRPC$Vector = lastResponse) == null) {
                ConnectionsManager.getInstance(i).sendRequest(tLRPC$TL_messages_getAttachedStickers, new RequestDelegate() { // from class: org.telegram.ui.Stories.StoryContainsEmojiButton$$ExternalSyntheticLambda2
                    @Override // org.telegram.tgnet.RequestDelegate
                    public final void run(TLObject tLObject2, TLRPC$TL_error tLRPC$TL_error) {
