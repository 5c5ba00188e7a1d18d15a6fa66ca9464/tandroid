...
public class VolumeSliderView extends View {
    ...
    */
    public boolean dispatchTouchEvent(MotionEvent motionEvent) {
        Utilities.Callback<Float> callback;
        if (this.w <= 0) {
            return false;
        }
        ...
        if (motionEvent.getAction() == 2 || motionEvent.getAction() == 1) {
            ...
            float clamp = Utilities.clamp(this.value + ((x - this.lastTouchX) / this.w), 1.0f, 0.0f);
            this.value = clamp;
            float f4 = this.maxVolume;
            float f5 = this.minVolume;
            float f6 = f4 - f5 != 0.0f ? f5 + (clamp * (f4 - f5)) : 0.0f;
            if ((f6 <= f5 && f3 > f6) || (f6 >= f4 && f3 < f6)) {
                performHapticFeedback(3, 1);
            } else {
                if (Math.floor(f3 * 5.0f) != Math.floor(5.0f * f6)) {
