...
class JsonUtils {
    ...
    public static PointF jsonToPoint(JsonReader jsonReader, float f) throws IOException {
        int i = 1.$SwitchMap$com$airbnb$lottie$parser$moshi$JsonReader$Token[jsonReader.peek().ordinal()];
        if (i != 1) {
            if (i != 2) {
                if (i == 3) {
    ...
    public static float valueFromObject(JsonReader jsonReader) throws IOException {
        ...
        int i = 1.$SwitchMap$com$airbnb$lottie$parser$moshi$JsonReader$Token[peek.ordinal()];
        if (i != 1) {
            if (i == 2) {
                jsonReader.beginArray();
                float nextDouble = (float) jsonReader.nextDouble();
