        ...
        */
        public void parseStartTag(XmlPullParser parser) throws ParserException {
            ...
            if (type == 2) {
                ...
                this.format = Format.createVideoContainerFormat(id, name, MimeTypes.VIDEO_MP4, sampleMimeType, null, null, bitrate, width, height, -1.0f, buildCodecSpecificData(parser.getAttributeValue(null, KEY_CODEC_PRIVATE_DATA)), 0, 0);
                return;
            }
            char c = 1;
            if (type == 1) {
                String sampleMimeType2 = sampleMimeType == null ? "audio/mp4a-latm" : sampleMimeType;
                ...
                if (codecSpecificData.isEmpty() && "audio/mp4a-latm".equals(sampleMimeType2)) {
                ...
                this.format = Format.createAudioContainerFormat(id, name, MimeTypes.AUDIO_MP4, sampleMimeType2, null, null, bitrate, channels, samplingRate, codecSpecificData, 0, 0, language);
            } else if (type == 3) {
                String subType = (String) getNormalizedAttribute(KEY_SUB_TYPE);
                switch (subType.hashCode()) {
                    case 2061026:
                        if (subType.equals("CAPT")) {
                            c = 0;
                            break;
                        }
                        c = 65535;
                ...
                this.format = Format.createTextContainerFormat(id, name, MimeTypes.APPLICATION_MP4, sampleMimeType, null, bitrate, 0, roleFlags, language2);
            } else {
