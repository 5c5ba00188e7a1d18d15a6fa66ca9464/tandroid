...
public final class DashMediaSource extends BaseMediaSource {
    ...
    */
    public DashMediaSource(Uri manifestUri, DataSource.Factory manifestDataSourceFactory, ParsingLoadable.Parser<? extends DashManifest> manifestParser, DashChunkSource.Factory chunkSourceFactory, int minLoadableRetryCount, long livePresentationDelayMs, Handler eventHandler, MediaSourceEventListener eventListener) {
        this(null, manifestUri, manifestDataSourceFactory, manifestParser, chunkSourceFactory, r8, r9, r10, r11, livePresentationDelayMs != -1, null);
        long j;
        DefaultCompositeSequenceableLoaderFactory defaultCompositeSequenceableLoaderFactory = new DefaultCompositeSequenceableLoaderFactory();
        DrmSessionManager dummyDrmSessionManager = DrmSessionManager.CC.getDummyDrmSessionManager();
        DefaultLoadErrorHandlingPolicy defaultLoadErrorHandlingPolicy = new DefaultLoadErrorHandlingPolicy(minLoadableRetryCount);
        if (livePresentationDelayMs == -1) {
            j = 30000;
        } else {
