...
public class MatroskaExtractor implements Extractor {
    ...
    */
    protected void binaryElement(int id, int contentSize, ExtractorInput input) throws IOException, InterruptedException {
        int i;
        int byteValue;
        int[] iArr;
        int i2 = 0;
        ...
        switch (id) {
            ...
            case ID_SIMPLE_BLOCK /* 163 */:
                ...
                if (this.blockState == 1) {
                    ...
                    if (lacing != 0) {
                        readScratch(input, 4);
                        int i4 = (this.scratch.data[3] & 255) + 1;
                        this.blockSampleCount = i4;
                        int[] ensureArrayCapacity = ensureArrayCapacity(this.blockSampleSizes, i4);
                        this.blockSampleSizes = ensureArrayCapacity;
                        if (lacing == 2) {
                            int i5 = this.blockSampleCount;
                            int blockLacingSampleSize = ((contentSize - this.blockTrackNumberLength) - 4) / i5;
                            Arrays.fill(ensureArrayCapacity, 0, i5, blockLacingSampleSize);
                        } else if (lacing != 1) {
                            if (lacing == 3) {
                                ...
                                int sampleIndex = 0;
                                while (true) {
                                    int i6 = this.blockSampleCount;
                                    if (sampleIndex >= i6 - 1) {
                                        this.blockSampleSizes[i6 - 1] = ((contentSize - this.blockTrackNumberLength) - headerSize) - totalSamplesSize;
                                    } else {
                                        this.blockSampleSizes[sampleIndex] = i2;
                                        headerSize++;
                                        ...
                                        while (true) {
                                            if (i7 < 8) {
                                                ...
                                                if ((this.scratch.data[headerSize - 1] & lengthMask) == 0) {
                                                    ...
                                                    headerSize += i7;
                                                    readScratch(input, headerSize);
                                                    readValue = this.scratch.data[readPosition] & 255 & (lengthMask ^ (-1));
                                                    for (int readPosition2 = readPosition + 1; readPosition2 < headerSize; readPosition2++) {
                                                        readValue = (readValue << 8) | (this.scratch.data[readPosition2] & 255);
                                                    }
                                                    if (sampleIndex > 0) {
                                                        readValue -= (1 << ((i7 * 7) + 6)) - 1;
                                                    }
                                                }
                                            }
                                        }
                                        if (readValue >= -2147483648L && readValue <= 2147483647L) {
                                            ...
                                            if (sampleIndex == 0) {
                                                ...
                                                i = iArr2[sampleIndex - 1] + intReadValue;
                                            }
                                            iArr2[sampleIndex] = i;
                                            totalSamplesSize += iArr2[sampleIndex];
                                            sampleIndex++;
                                            i2 = 0;
                                            i3 = 1;
                                        }
                            ...
                            int sampleIndex2 = 0;
                            while (true) {
                                int i8 = this.blockSampleCount;
                                if (sampleIndex2 >= i8 - 1) {
                                    this.blockSampleSizes[i8 - 1] = ((contentSize - this.blockTrackNumberLength) - headerSize2) - totalSamplesSize2;
                                } else {
                                    this.blockSampleSizes[sampleIndex2] = 0;
                                    do {
                                        headerSize2++;
                                        readScratch(input, headerSize2);
                                        byteValue = this.scratch.data[headerSize2 - 1] & 255;
                                        iArr = this.blockSampleSizes;
                                        iArr[sampleIndex2] = iArr[sampleIndex2] + byteValue;
                                    } while (byteValue == 255);
                                    totalSamplesSize2 += iArr[sampleIndex2];
                                    sampleIndex2++;
                                }
                            }
                        }
                        ...
                        int[] ensureArrayCapacity2 = ensureArrayCapacity(this.blockSampleSizes, 1);
                        this.blockSampleSizes = ensureArrayCapacity2;
                        ensureArrayCapacity2[0] = (contentSize - this.blockTrackNumberLength) - 3;
                    }
                    ...
                    this.blockFlags = (isKeyframe ? 1 : 0) | (isInvisible ? Integer.MIN_VALUE : 0);
                    this.blockState = 2;
                    this.blockSampleIndex = 0;
                }
                if (id == ID_SIMPLE_BLOCK) {
                    while (true) {
                        int i9 = this.blockSampleIndex;
                        if (i9 < this.blockSampleCount) {
                            int sampleSize = writeSampleData(input, track, this.blockSampleSizes[i9]);
                            long sampleTimeUs = this.blockTimeUs + ((this.blockSampleIndex * track.defaultSampleDurationNs) / 1000);
                            commitSampleToOutput(track, sampleTimeUs, this.blockFlags, sampleSize, 0);
                    ...
                    while (true) {
                        int i10 = this.blockSampleIndex;
                        if (i10 < this.blockSampleCount) {
                            int[] iArr3 = this.blockSampleSizes;
                            iArr3[i10] = writeSampleData(input, track, iArr3[i10]);
                            this.blockSampleIndex++;
                        } else {
                            return;
                        }
