...
public class DefaultMp4Builder implements Mp4Builder {
    ...
    */
    protected MovieBox createMovieBox(Movie movie, Map<Track, int[]> chunks) {
        ...
        MovieHeaderBox mvhd2 = new MovieHeaderBox();
        mvhd2.setCreationTime(new Date());
        mvhd2.setModificationTime(new Date());
        mvhd2.setMatrix(movie.getMatrix());
        ...
        for (Track track : movie.getTracks()) {
            if (track.getEdits() == null) {
                movieBox = movieBox2;
                mvhd = mvhd2;
            } else if (track.getEdits().isEmpty()) {
                movieBox = movieBox2;
                mvhd = mvhd2;
            } else {
                long d = 0;
                ...
                tracksDuration = getTimescale(movie) * d;
                movieBox = movieBox2;
                mvhd = mvhd2;
        ...
        mvhd2.setDuration(duration);
        mvhd2.setTimescale(movieTimeScale);
        long nextTrackId = 0;
        for (Track track2 : movie.getTracks()) {
            nextTrackId = nextTrackId < track2.getTrackMetaData().getTrackId() ? track2.getTrackMetaData().getTrackId() : nextTrackId;
        }
        mvhd2.setNextTrackId(1 + nextTrackId);
        movieBox2.addBox(mvhd2);
        for (Track track3 : movie.getTracks()) {
            movieBox2.addBox(createTrackBox(track3, movie, chunks));
        }
        Box udta = createUdta(movie);
        if (udta != null) {
            movieBox2.addBox(udta);
        }
        return movieBox2;
    }
