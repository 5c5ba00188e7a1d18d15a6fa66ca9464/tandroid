        ...
        public int parse(DateTimeParseContext context, CharSequence text, int position) {
            ...
            if (sign == '+' || sign == '-') {
                ...
                if (!parseNumber(array, 1, text, true)) {
                    if (!parseNumber(array, 2, text, this.type >= 3) && !parseNumber(array, 3, text, false)) {
                        z = false;
                        if (!z) {
    ...
    public static final class ZoneTextPrinterParser extends ZoneIdPrinterParser {
        ...
        */
        private String getDisplayName(String id, int type, Locale locale) {
            ...
            Map<String, SoftReference<Map<Locale, String[]>>> map = cache;
            SoftReference<Map<Locale, String[]>> ref = map.get(id);
            Map<Locale, String[]> perLocale = null;
            if (ref != null) {
                Map<Locale, String[]> map2 = ref.get();
                perLocale = map2;
                if (map2 != null) {
            ...
            map.put(id, new SoftReference<>(perLocale));
            switch (type) {
                case 0:
