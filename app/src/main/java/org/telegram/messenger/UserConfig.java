    ...
    public void loadConfig() {
        ...
        synchronized (this.sync) {
            ...
            if (!preferences.contains("2dialogsLoadOffsetId") && !preferences.getBoolean("hasValidDialogLoadIds", false)) {
                ...
                terms = preferences.getString("terms", null);
            ...
            terms = preferences.getString("terms", null);
            if (terms != null) {
                SerializedData data4 = new SerializedData(arr);
                this.unacceptedTermsOfService = TLRPC.TL_help_termsOfService.TLdeserialize(data4, data4.readInt32(false), false);
                data4.cleanup();
            }
            i = preferences.getInt("6migrateOffsetId", 0);
            this.migrateOffsetId = i;
            ...
            string = preferences.getString("tmpPassword", null);
            if (string != null) {
                SerializedData data22 = new SerializedData(bytes2);
                this.tmpPassword = TLRPC.TL_account_tmpPassword.TLdeserialize(data22, data22.readInt32(false), false);
                data22.cleanup();
            }
            string2 = preferences.getString("user", null);
            if (string2 != null) {
                SerializedData data32 = new SerializedData(bytes);
                this.currentUser = TLRPC.User.TLdeserialize(data32, data32.readInt32(false), false);
                data32.cleanup();
            }
            user = this.currentUser;
            if (user != null) {
