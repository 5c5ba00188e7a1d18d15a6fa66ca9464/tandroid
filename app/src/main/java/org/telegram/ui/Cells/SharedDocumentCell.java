...
public class SharedDocumentCell extends FrameLayout implements DownloadController.FileDownloadProgressListener {
    ...
    */
    public void setDocument(MessageObject messageObject, boolean divider) {
        ...
        boolean animated = (messageObject2 == null || messageObject == null || messageObject2.getId() == messageObject.getId()) ? false : true;
        this.needDivider = divider;
        ...
        if (document == null) {
            ...
            if (!messageObject.isVideo() && !(messageObject.messageOwner.media instanceof TLRPC.TL_messageMediaPhoto) && !MessageObject.isGifDocument(document)) {
                fileName2 = FileLoader.getDocumentFileName(document);
            }
            if (TextUtils.isEmpty(fileName2) && document.mime_type != null) {
                ...
                } else if (document.mime_type.startsWith(TtmlNode.TAG_IMAGE)) {
                    if (MessageObject.isGifDocument(document)) {
                        fileName = LocaleController.getString("AttachGif", R.string.AttachGif);
                    } else {
            ...
            CharSequence nameH = AndroidUtilities.highlightText(name, messageObject.highlightedWords, this.resourcesProvider);
            if (nameH != null) {
                this.nameTextView.setText(nameH);
            } else {
            ...
            TLRPC.PhotoSize bigthumb2 = FileLoader.getClosestPhotoSizeWithSize(document.thumbs, GroupCallActivity.TABLET_LIST_SIZE);
            ...
            if (!(thumb instanceof TLRPC.TL_photoSizeEmpty) && thumb != null) {
                ...
                if (messageObject.strippedThumb != null) {
                    this.thumbImageView.setImage(ImageLocation.getForDocument(bigthumb, document), "40_40", null, null, messageObject.strippedThumb, null, null, 1, messageObject);
                } else {
                    this.thumbImageView.setImage(ImageLocation.getForDocument(bigthumb, document), "40_40", ImageLocation.getForDocument(thumb, document), "40_40_b", null, 0L, 1, messageObject);
                }
                updateDateView();
                if (!messageObject.hasHighlightedWords() && !TextUtils.isEmpty(this.message.messageOwner.message)) {
                    String str2 = this.message.messageOwner.message.replace("\n", " ").replaceAll(" +", " ").trim();
                    CharSequence highlightText = AndroidUtilities.highlightText(str2, this.message.highlightedWords, this.resourcesProvider);
                    this.caption = highlightText;
                    TextView textView4 = this.captionTextView;
                    if (textView4 != null) {
                        textView4.setVisibility(highlightText == null ? 8 : 0);
                    }
                } else {
            ...
            this.placeholderImageView.setAlpha(1.0f);
            updateDateView();
            if (!messageObject.hasHighlightedWords()) {
            }
            textView = this.captionTextView;
            if (textView != null) {
            }
