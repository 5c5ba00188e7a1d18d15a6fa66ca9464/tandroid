    ...
    public static class AvatarWavesDrawable {
        ...
        */
        public void draw(Canvas canvas, float cx, float cy, View parentView) {
            float scaleBlob = (this.amplitude * 0.4f) + 0.8f;
            if (this.showWaves || this.wavesEnter != 0.0f) {
                ...
                if (!this.hasCustomColor) {
                    int i = this.isMuted;
                    if (i != 1) {
                        float f = this.progressToMuted;
                        if (f != 1.0f) {
                            float f2 = f + 0.10666667f;
                            this.progressToMuted = f2;
                            if (f2 > 1.0f) {
                                this.progressToMuted = 1.0f;
                            }
                            this.invalidateColor = true;
                            if (this.invalidateColor) {
                                int color = ColorUtils.blendARGB(Theme.getColor(Theme.key_voipgroup_speakingText), Theme.getColor(this.isMuted == 2 ? Theme.key_voipgroup_mutedByAdminIcon : Theme.key_voipgroup_listeningText), this.progressToMuted);
                                this.blobDrawable.paint.setColor(ColorUtils.setAlphaComponent(color, 38));
                            }
                        }
                    }
                    if (i == 1) {
                        float f3 = this.progressToMuted;
                        if (f3 != 0.0f) {
                ...
                this.blobDrawable2.draw(cx, cy, canvas, this.blobDrawable.paint);
                canvas.restore();
            }
