    ...
    */
    public void onDonePressed() {
        ...
        if (!ChatObject.isChannel(this.currentChat)) {
            ...
            MessagesController.getInstance(this.currentAccount).convertToMegaGroup(getParentActivity(), this.chatId, this, new MessagesStorage.LongCallback() { // from class: org.telegram.ui.ChatRightsEditActivity$$ExternalSyntheticLambda12
                @Override // org.telegram.messenger.MessagesStorage.LongCallback
                public final void run(long j) {
                    ChatRightsEditActivity.this.m2138lambda$onDonePressed$15$orgtelegramuiChatRightsEditActivity(j);
                }
            });
            return;
        }
        ...
        if (i2 == 0 || i2 == 2) {
            if (this.rankRow != -1) {
                ...
                if (str4.codePointCount(0, str4.length()) > 16) {
                    ...
                    RecyclerView.ViewHolder holder = this.listView.findViewHolderForAdapterPosition(this.rankHeaderRow);
                    if (holder != null) {
                        AndroidUtilities.shakeView(holder.itemView, 2.0f, 0);
                        return;
                    }
                    return;
                }
            ...
            if (!this.adminRights.change_info && !this.adminRights.post_messages && !this.adminRights.edit_messages && !this.adminRights.delete_messages && !this.adminRights.ban_users && !this.adminRights.invite_users && !this.adminRights.pin_messages && !this.adminRights.add_admins && !this.adminRights.anonymous && !this.adminRights.manage_call) {
                this.adminRights.other = true;
            } else {
                this.adminRights.other = false;
            }
        }
        boolean finishFragment = true;
        int rights2 = this.currentType;
        if (rights2 == 0) {
            ...
            finishFragment = z;
            setLoading(true);
            MessagesController.getInstance(this.currentAccount).setUserAdminRole(this.chatId, this.currentUser, this.adminRights, this.currentRank, this.isChannel, this, this.isAddingNew, false, null, new Runnable() { // from class: org.telegram.ui.ChatRightsEditActivity$$ExternalSyntheticLambda4
                @Override // java.lang.Runnable
                public final void run() {
                    ChatRightsEditActivity.this.m2139lambda$onDonePressed$16$orgtelegramuiChatRightsEditActivity();
                }
            }, new MessagesController.ErrorDelegate() { // from class: org.telegram.ui.ChatRightsEditActivity$$ExternalSyntheticLambda8
                @Override // org.telegram.messenger.MessagesController.ErrorDelegate
                public final boolean run(TLRPC.TL_error tL_error) {
                    return ChatRightsEditActivity.this.m2140lambda$onDonePressed$17$orgtelegramuiChatRightsEditActivity(tL_error);
                }
            });
        } else if (rights2 == 1) {
            ...
            ChatRightsEditActivityDelegate chatRightsEditActivityDelegate = this.delegate;
            if (chatRightsEditActivityDelegate != null) {
                chatRightsEditActivityDelegate.didSetRights(rights, this.adminRights, this.bannedRights, this.currentRank);
            }
        } else if (rights2 == 2) {
            AlertDialog.Builder builder = new AlertDialog.Builder(getParentActivity());
            if (this.asAdmin) {
            ...
            boolean isChannel = ChatObject.isChannel(this.currentChat) && !this.currentChat.megagroup;
            TLRPC.Chat chat = this.currentChat;
            String chatName = chat == null ? "" : chat.title;
            if (this.asAdmin) {
                if (isChannel) {
                    str2 = LocaleController.formatString("AddBotMessageAdminChannel", R.string.AddBotMessageAdminChannel, chatName);
                } else {
                    str2 = LocaleController.formatString("AddBotMessageAdminGroup", R.string.AddBotMessageAdminGroup, chatName);
                }
            } else {
                str2 = LocaleController.formatString("AddMembersAlertNamesText", R.string.AddMembersAlertNamesText, UserObject.getUserName(this.currentUser), chatName);
            }
            builder.setMessage(AndroidUtilities.replaceTags(str2));
            builder.setNegativeButton(LocaleController.getString("Cancel", R.string.Cancel), null);
            builder.setPositiveButton(this.asAdmin ? LocaleController.getString("AddAsAdmin", R.string.AddAsAdmin) : LocaleController.getString("AddBot", R.string.AddBot), new DialogInterface.OnClickListener() { // from class: org.telegram.ui.ChatRightsEditActivity$$ExternalSyntheticLambda25
                @Override // android.content.DialogInterface.OnClickListener
                public final void onClick(DialogInterface dialogInterface, int i3) {
                    ChatRightsEditActivity.this.m2144lambda$onDonePressed$21$orgtelegramuiChatRightsEditActivity(dialogInterface, i3);
                }
            });
            showDialog(builder.create());
