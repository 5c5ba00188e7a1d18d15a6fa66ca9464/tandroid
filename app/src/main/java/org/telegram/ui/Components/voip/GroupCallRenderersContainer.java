    ...
    */
    protected void dispatchDraw(Canvas canvas) {
        ...
        if (this.outFullscreenTextureView != null || this.fullscreenTextureView != null) {
            ...
            float progress = this.progressToFullscreenMode;
            canvas.save();
            if (!GroupCallActivity.isTabletMode && (groupCallMiniTextureView2 = this.fullscreenTextureView) != null && !groupCallMiniTextureView2.forceDetached && this.fullscreenTextureView.primaryView != null) {
            ...
            GroupCallMiniTextureView groupCallMiniTextureView3 = this.outFullscreenTextureView;
            if (groupCallMiniTextureView3 != null && groupCallMiniTextureView3.getParent() != null) {
                canvas.save();
            ...
            if (groupCallMiniTextureView4 != null && groupCallMiniTextureView4.getParent() != null) {
                if (this.fullscreenTextureView.getAlpha() != 1.0f) {
                    AndroidUtilities.rectTmp.set(this.fullscreenTextureView.getX(), this.fullscreenTextureView.getY(), this.fullscreenTextureView.getX() + this.fullscreenTextureView.getMeasuredWidth(), this.fullscreenTextureView.getY() + this.fullscreenTextureView.getMeasuredHeight());
                    canvas.saveLayerAlpha(AndroidUtilities.rectTmp, (int) (this.fullscreenTextureView.getAlpha() * 255.0f), 31);
                } else {
                    canvas.save();
                }
                boolean swipeToBack = this.swipeToBackGesture || this.swipeToBackAnimator != null;
                ...
                this.fullscreenTextureView.setSwipeToBack(swipeToBack, this.swipeToBackDy);
        ...
        for (int i = 0; i < 2; i++) {
            if (this.undoView[i].getVisibility() == 0) {
                canvas.save();
                float offset = GroupCallActivity.isLandscapeMode ? 0.0f : (-AndroidUtilities.dp(90.0f)) * (1.0f - this.progressToHideUi);
                canvas.clipRect(0.0f, 0.0f, getMeasuredWidth(), ((getMeasuredHeight() - (GroupCallActivity.isLandscapeMode ? 0 : AndroidUtilities.dp(90.0f))) + offset) - AndroidUtilities.dp(18.0f));
                if (this.isTablet) {
                    canvas.translate(this.undoView[i].getX() - AndroidUtilities.dp(8.0f), this.undoView[i].getY() - AndroidUtilities.dp(8.0f));
                } else {
                    canvas.translate(this.undoView[i].getX() - AndroidUtilities.dp(8.0f), ((this.undoView[i].getY() - (GroupCallActivity.isLandscapeMode ? 0 : AndroidUtilities.dp(90.0f))) + offset) - AndroidUtilities.dp(26.0f));
                }
        ...
        float a = this.progressToFullscreenMode * (1.0f - this.progressToHideUi);
        if (this.replaceFullscreenViewAnimator == null || (groupCallMiniTextureView = this.outFullscreenTextureView) == null || this.fullscreenTextureView == null) {
            GroupCallMiniTextureView groupCallMiniTextureView5 = this.fullscreenTextureView;
            if (groupCallMiniTextureView5 != null) {
                this.topShadowDrawable.setAlpha((int) (a * 255.0f * (1.0f - groupCallMiniTextureView5.progressToNoVideoStub)));
                this.rightShadowDrawable.setAlpha((int) (255.0f * a * (1.0f - this.fullscreenTextureView.progressToNoVideoStub)));
            } else {
                this.topShadowDrawable.setAlpha((int) (a * 255.0f));
                this.rightShadowDrawable.setAlpha((int) (255.0f * a));
            }
        } else {
            float shadowAlpha = a;
            if (groupCallMiniTextureView.hasVideo != this.fullscreenTextureView.hasVideo) {
                if (!this.fullscreenTextureView.hasVideo) {
                    shadowAlpha *= 1.0f - this.fullscreenTextureView.getAlpha();
                } else {
                    shadowAlpha *= this.fullscreenTextureView.getAlpha();
                }
            } else if (!this.fullscreenTextureView.hasVideo) {
                shadowAlpha = 0.0f;
            }
            this.topShadowDrawable.setAlpha((int) (shadowAlpha * 255.0f));
            this.rightShadowDrawable.setAlpha((int) (255.0f * shadowAlpha));
        }
        this.backButton.setAlpha(a);
        if (isRtmpStream()) {
            this.pinButton.setAlpha(0.0f);
            this.pinButton.setVisibility(4);
            this.pipView.setAlpha(a);
            ...
            this.pinButton.setAlpha(a);
            this.pinButton.setVisibility(0);
            this.pipView.setAlpha(0.0f);
            this.pipView.setVisibility(4);
        }
        float x1 = getMeasuredWidth() - this.pinTextView.getMeasuredWidth();
        float x2 = getMeasuredWidth() - this.unpinTextView.getMeasuredWidth();
        float pinY = ((ActionBar.getCurrentActionBarHeight() - this.pinTextView.getMeasuredHeight()) / 2.0f) - AndroidUtilities.dp(1.0f);
        float pinX2 = ((this.pinDrawable.getProgress() * x2) + ((1.0f - this.pinDrawable.getProgress()) * x1)) - AndroidUtilities.dp(21.0f);
        if (GroupCallActivity.isTabletMode) {
            pinX = pinX2 - AndroidUtilities.dp(328.0f);
        } else {
            pinX = pinX2 - (GroupCallActivity.isLandscapeMode ? AndroidUtilities.dp(180.0f) : 0);
        }
        this.pinTextView.setTranslationX(pinX);
        this.unpinTextView.setTranslationX(pinX);
        this.pinTextView.setTranslationY(pinY);
        this.unpinTextView.setTranslationY(pinY);
        this.pinContainer.setTranslationX(pinX - AndroidUtilities.dp(36.0f));
        this.pinContainer.setTranslationY((ActionBar.getCurrentActionBarHeight() - this.pinContainer.getMeasuredHeight()) / 2.0f);
        this.pinButton.setTranslationX(pinX - AndroidUtilities.dp(44.0f));
        if (!isRtmpStream()) {
            this.pinTextView.setAlpha((1.0f - this.pinDrawable.getProgress()) * a);
            this.unpinTextView.setAlpha(this.pinDrawable.getProgress() * a);
            this.pinContainer.setAlpha(a);
        } else {
            this.pinTextView.setAlpha(0.0f);
            this.unpinTextView.setAlpha(0.0f);
            this.pinContainer.setAlpha(0.0f);
        }
        float f = this.speakingMembersToastChangeProgress;
        if (f != 1.0f) {
            float f2 = f + 0.07272727f;
            this.speakingMembersToastChangeProgress = f2;
            if (f2 > 1.0f) {
        ...
        if (z) {
            float f3 = this.showSpeakingMembersToastProgress;
            if (f3 != 1.0f) {
                float f4 = f3 + 0.10666667f;
                this.showSpeakingMembersToastProgress = f4;
                if (f4 > 1.0f) {
                ...
                if (this.fullscreenListView.getVisibility() != 0) {
                    for (int i2 = 0; i2 < this.fullscreenListView.getChildCount(); i2++) {
                        GroupCallFullscreenAdapter.GroupCallUserCell child = (GroupCallFullscreenAdapter.GroupCallUserCell) this.fullscreenListView.getChildAt(i2);
                        if (child.getVisibility() == 0 && child.getAlpha() != 0.0f) {
                            canvas.save();
                            canvas.translate(child.getX() + this.fullscreenListView.getX(), child.getY() + this.fullscreenListView.getY());
                            canvas.scale(child.getScaleX(), child.getScaleY(), child.getMeasuredWidth() / 2.0f, child.getMeasuredHeight() / 2.0f);
        ...
        if (!z) {
            float f5 = this.showSpeakingMembersToastProgress;
            if (f5 != 0.0f) {
                float f6 = f5 - 0.10666667f;
                this.showSpeakingMembersToastProgress = f6;
                if (f6 < 0.0f) {
        ...
        this.speakingMembersToast.setScaleX((this.showSpeakingMembersToastProgress * 0.5f) + 0.5f);
        this.speakingMembersToast.setScaleY((this.showSpeakingMembersToastProgress * 0.5f) + 0.5f);
        boolean z22 = GroupCallActivity.isTabletMode;
        if (!GroupCallActivity.isTabletMode) {
        }
        if (this.fullscreenListView.getVisibility() != 0) {
        }
