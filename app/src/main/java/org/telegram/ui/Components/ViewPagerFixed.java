    ...
    */
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        if (tabsView == null || !tabsView.animatingIndicator) {
            ...
            if (!this.startedTracking && ev != null && (child = findScrollingChild(this, ev.getX(), ev.getY())) != null && (child.canScrollHorizontally(1) || child.canScrollHorizontally(-1))) {
                return false;
            }
            if (ev == null || ev.getAction() != 0 || this.startedTracking || this.maybeStartTracking) {
                if (ev == null || ev.getAction() != 2 || ev.getPointerId(0) != this.startedTrackingPointerId) {
                    if (ev == null || (ev.getPointerId(0) == this.startedTrackingPointerId && (ev.getAction() == 3 || ev.getAction() == 1 || ev.getAction() == 6))) {
                        ...
                        if (this.startedTracking) {
                            ...
                            if (this.additionalOffset == 0.0f) {
                                ...
                                boolean z3 = this.animatingForward ? false : false;
                            ...
                            if (this.backAnimation) {
                                dx = Math.abs(x2);
                                if (this.animatingForward) {
                                    this.tabsAnimation.playTogether(ObjectAnimator.ofFloat(this.viewPages[0], View.TRANSLATION_X, 0.0f), ObjectAnimator.ofFloat(this.viewPages[1], View.TRANSLATION_X, this.viewPages[1].getMeasuredWidth()));
                                } else {
                                    this.tabsAnimation.playTogether(ObjectAnimator.ofFloat(this.viewPages[0], View.TRANSLATION_X, 0.0f), ObjectAnimator.ofFloat(this.viewPages[1], View.TRANSLATION_X, -this.viewPages[1].getMeasuredWidth()));
                                }
                            } else {
                                dx = this.viewPages[0].getMeasuredWidth() - Math.abs(x2);
                                if (this.animatingForward) {
                                    this.tabsAnimation.playTogether(ObjectAnimator.ofFloat(this.viewPages[0], View.TRANSLATION_X, -this.viewPages[0].getMeasuredWidth()), ObjectAnimator.ofFloat(this.viewPages[1], View.TRANSLATION_X, 0.0f));
                                } else {
                                    this.tabsAnimation.playTogether(ObjectAnimator.ofFloat(this.viewPages[0], View.TRANSLATION_X, this.viewPages[0].getMeasuredWidth()), ObjectAnimator.ofFloat(this.viewPages[1], View.TRANSLATION_X, 0.0f));
                                }
                            }
                            ...
                            this.tabsAnimation.playTogether(animator);
                    ...
                    int dx2 = (int) ((ev.getX() - this.startedTrackingX) + this.additionalOffset);
                    ...
                    if (this.maybeStartTracking && !this.startedTracking) {
                        ...
                        if (Math.abs(dxLocal) >= this.touchSlop && Math.abs(dxLocal) > dy) {
                            prepareForMoving(ev, dx2 < 0);
                        }
                    } else if (this.startedTracking) {
                        this.viewPages[0].setTranslationX(dx2);
                        if (this.animatingForward) {
                            View[] viewArr2 = this.viewPages;
                            viewArr2[1].setTranslationX(viewArr2[0].getMeasuredWidth() + dx2);
                        } else {
                            View[] viewArr3 = this.viewPages;
                            viewArr3[1].setTranslationX(dx2 - viewArr3[0].getMeasuredWidth());
                        }
                        float scrollProgress = Math.abs(dx2) / this.viewPages[0].getMeasuredWidth();
                    ...
                    if (f != 1.0f) {
                        ...
                        if (this.animatingIndicator && this.manualScrollingToPosition == -1) {
                            ...
                            if (holder != null) {
                                ...
                                indicatorWidth = Math.max(AndroidUtilities.dp(40.0f), tabView.tabWidth);
                                indicatorX = (int) (tabView.getX() + ((tabView.getMeasuredWidth() - indicatorWidth) / 2));
                            }
                        } else {
                            position = this.layoutManager.findFirstVisibleItemPosition();
                            if (position != -1) {
                                indicatorX2 = 0;
                                indicatorWidth2 = 0;
                            } else {
                                ...
                                if (holder2 == null) {
                                    ...
                                    if (this.additionalTabWidth != 0) {
                                        x = ((int) (prevX + ((newX - prevX) * this.animatingIndicatorProgress))) + AndroidUtilities.dp(16.0f);
                                    } else {
                                        int x2 = this.positionToX.get(position);
                                        x = (((int) (prevX + ((newX - prevX) * this.animatingIndicatorProgress))) - (x2 - holder2.itemView.getLeft())) + AndroidUtilities.dp(16.0f);
                                    }
                                    indicatorX = x;
                                    indicatorWidth = (int) (prevW + ((newW - prevW) * this.animatingIndicatorProgress));
                                }
                            }
                        ...
                        if (indicatorWidth != 0) {
                            this.selectorDrawable.setBounds(indicatorX, (int) ((height - AndroidUtilities.dpr(4.0f)) + (this.hideProgress * AndroidUtilities.dpr(4.0f))), indicatorX + indicatorWidth, (int) (height + (this.hideProgress * AndroidUtilities.dpr(4.0f))));
