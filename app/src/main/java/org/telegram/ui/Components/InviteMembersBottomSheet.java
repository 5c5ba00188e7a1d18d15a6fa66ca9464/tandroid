    ...
    public class SearchAdapter extends RecyclerListView.SelectionAdapter {
        ...
        */
        public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
            ...
            switch (holder.getItemViewType()) {
                case 0:
                    ((GroupCreateSectionCell) holder.itemView).setText(LocaleController.getString("GlobalSearch", R.string.GlobalSearch));
                    break;
                case 1:
                    GroupCreateUserCell cell = (GroupCreateUserCell) holder.itemView;
                    ...
                    int position2 = position - 1;
                    if (position2 >= 0 && position2 < localCount) {
                        object = (TLObject) this.searchResult.get(position2);
                    } else if (position2 >= localCount && position2 < localServerCount + localCount) {
                        object = this.searchAdapterHelper.getLocalServerSearch().get(position2 - localCount);
                    } else if (position2 > localCount + localServerCount && position2 <= globalCount + localCount + localServerCount) {
                        object = this.searchAdapterHelper.getGlobalSearch().get(((position2 - localCount) - localServerCount) - 1);
                    } else {
                        object = null;
                    }
                    if (object == null) {
                        ...
                        if (position2 < localCount) {
                            name = this.searchResultNames.get(position2);
                        ...
                        } else if (position2 <= localCount || TextUtils.isEmpty(objectUserName)) {
                            username = null;
                        } else {
                            ...
                            try {
                                ...
                                int indexOfIgnoreCase = AndroidUtilities.indexOfIgnoreCase(objectUserName, foundUserName);
                                ...
                                if (indexOfIgnoreCase != -1) {
                                    ...
                                    try {
                                        ...
                                        oldObject = cell.getObject();
                                        if (oldObject instanceof TLRPC.User) {
                                        }
                                        cell.setObject(object, name, username2);
                                        if (object instanceof TLRPC.User) {
                                        }
                                        if (id == 0) {
                                        }
        ...
        /*
            Code decompiled incorrectly, please refer to instructions dump.
        */
        public /* synthetic */ void m2715x50c47b38(String query) {
            String name;
            String username;
            String search1;
            ...
            int a = 0;
            while (a < InviteMembersBottomSheet.this.contacts.size()) {
                TLObject object = (TLObject) InviteMembersBottomSheet.this.contacts.get(a);
                if (object instanceof TLRPC.User) {
