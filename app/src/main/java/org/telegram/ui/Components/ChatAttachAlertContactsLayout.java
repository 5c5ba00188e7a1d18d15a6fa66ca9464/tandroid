...
public class ChatAttachAlertContactsLayout extends ChatAttachAlert.AttachAlertLayout implements NotificationCenter.NotificationCenterDelegate {
    ...
    public class ShareSearchAdapter extends RecyclerListView.SelectionAdapter {
        ...
        */
        public /* synthetic */ void m2428xcb356c14(String query, ArrayList contactsCopy, ArrayList contactsCopy2, int currentAccount, int searchId) {
            ...
            if (search12.length() == 0) {
                this.lastSearchId = -1;
                updateSearchResults(query, new ArrayList<>(), new ArrayList<>(), this.lastSearchId);
                return;
            }
            ...
            while (a < contactsCopy.size()) {
                ContactsController.Contact contact2 = (ContactsController.Contact) contactsCopy.get(a);
                String name = ContactsController.formatName(contact2.first_name, contact2.last_name).toLowerCase();
                String tName = LocaleController.getInstance().getTranslitString(name);
                if (contact2.user != null) {
                    search1 = search12;
                    name2 = ContactsController.formatName(contact2.user.first_name, contact2.user.last_name).toLowerCase();
                    ...
                    if (i < length) {
                        ...
                        if (name2 != null) {
                            ...
                            if (found == 0) {
                                ...
                                if (found == 3) {
                                    resultArrayNames.add(AndroidUtilities.generateSearchName(contact2.first_name, contact2.last_name, q));
                                } else if (found == 1) {
                                    resultArrayNames.add(AndroidUtilities.generateSearchName(contact2.user.first_name, contact2.user.last_name, q));
                                } else {
                                    resultArrayNames.add(AndroidUtilities.generateSearchName("@" + contact2.user.username, null, "@" + q));
                                }
                                if (contact2.user != null) {
                                    foundUids.put(contact2.user.id, 1);
                                }
                                resultArray.add(contact2);
                            }
                        }
                        if (tName2 != null) {
                            if (!tName2.startsWith(q)) {
                            }
                            found = 1;
                            if (found == 0) {
                            }
                        }
                        if (contact2.user != null && contact2.user.username != null && contact2.user.username.startsWith(q)) {
                            found = 2;
                        } else {
            ...
            for (int a2 = 0; a2 < contactsCopy2.size(); a2++) {
                TLRPC.TL_contact contact3 = (TLRPC.TL_contact) contactsCopy2.get(a2);
                if (foundUids.indexOfKey(contact3.user_id) < 0) {
                    TLRPC.User user = MessagesController.getInstance(currentAccount).getUser(Long.valueOf(contact3.user_id));
                    String name3 = ContactsController.formatName(user.first_name, user.last_name).toLowerCase();
                    String tName3 = LocaleController.getInstance().getTranslitString(name3);
                    if (name3.equals(tName3)) {
                        tName3 = null;
                    }
                    int found3 = 0;
                    ...
                    while (true) {
                        if (i3 < length2) {
                            String q2 = search[i3];
                            if (!name3.startsWith(q2)) {
                                contact = contact3;
                                if (!name3.contains(" " + q2)) {
                                    if (tName3 != null) {
                                        if (!tName3.startsWith(q2)) {
                                        }
                                    }
                                    if (user.username != null && user.username.startsWith(q2)) {
                                        found3 = 2;
                                    }
                                    if (found3 != 0) {
                                        i3++;
                                        ...
                                        if (found3 == 1) {
                                            resultArrayNames.add(AndroidUtilities.generateSearchName(user.first_name, user.last_name, q2));
                                        } else {
                                            resultArrayNames.add(AndroidUtilities.generateSearchName("@" + user.username, null, "@" + q2));
                                        }
