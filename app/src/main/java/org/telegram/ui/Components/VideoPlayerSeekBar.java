...
public class VideoPlayerSeekBar {
    ...
    */
    private void drawProgressBar(Canvas canvas, RectF rect, Paint paint2) {
        float lineWidth;
        float left;
        char c;
        VideoPlayerSeekBar videoPlayerSeekBar = this;
        float radius = AndroidUtilities.dp(AndroidUtilities.lerp(2, 1, videoPlayerSeekBar.transitionProgress));
        ArrayList<Pair<Float, CharSequence>> arrayList = videoPlayerSeekBar.timestamps;
        if (arrayList == null || arrayList.isEmpty()) {
            canvas.drawRoundRect(rect, radius, radius, paint2);
            return;
        }
        float lineWidth2 = rect.bottom - rect.top;
        float left2 = videoPlayerSeekBar.horizontalPadding + AndroidUtilities.lerp(thumbWidth / 2.0f, 0.0f, videoPlayerSeekBar.transitionProgress);
        float right = videoPlayerSeekBar.horizontalPadding + AndroidUtilities.lerp(videoPlayerSeekBar.width - (thumbWidth / 2.0f), videoPlayerSeekBar.parentView.getWidth() - (videoPlayerSeekBar.horizontalPadding * 2.0f), videoPlayerSeekBar.transitionProgress);
        AndroidUtilities.rectTmp.set(rect);
        float halfGap = AndroidUtilities.dp(videoPlayerSeekBar.timestampsAppearing * 1.0f) / 2.0f;
        if (tmpPath == null) {
            tmpPath = new Path();
        }
        tmpPath.reset();
        float minDur = AndroidUtilities.dp(4.0f) / (right - left2);
        int start = -1;
        int end = -1;
        int i = 0;
        while (true) {
            if (i >= videoPlayerSeekBar.timestamps.size()) {
                break;
            } else if (((Float) videoPlayerSeekBar.timestamps.get(i).first).floatValue() < minDur) {
                i++;
            } else {
                start = i;
                break;
        ...
        int i2 = videoPlayerSeekBar.timestamps.size() - 1;
        while (true) {
            if (i2 < 0) {
                break;
            } else if (1.0f - ((Float) videoPlayerSeekBar.timestamps.get(i2).first).floatValue() < minDur) {
                i2--;
            } else {
                end = i2 + 1;
                break;
        ...
        if (end < 0) {
            end = videoPlayerSeekBar.timestamps.size();
        }
        boolean first = true;
        int i3 = start;
        while (i3 <= end) {
            float from = i3 == start ? 0.0f : ((Float) videoPlayerSeekBar.timestamps.get(i3 - 1).first).floatValue();
            float to = i3 == end ? 1.0f : ((Float) videoPlayerSeekBar.timestamps.get(i3).first).floatValue();
            boolean first2 = first;
            AndroidUtilities.rectTmp.left = AndroidUtilities.lerp(left2, right, from) + (i3 > 0 ? halfGap : 0.0f);
            AndroidUtilities.rectTmp.right = AndroidUtilities.lerp(left2, right, to) - (i3 < end ? halfGap : 0.0f);
            boolean z = AndroidUtilities.rectTmp.right > rect.right;
            boolean last = z;
            if (!z) {
                ...
                lineWidth = lineWidth2;
                float lineWidth3 = rect.right;
                rectF.right = lineWidth3;
            }
            if (AndroidUtilities.rectTmp.right < rect.left) {
                ...
                if (i3 == start) {
                    left = left2;
                    c = 1;
                } else if (!last || AndroidUtilities.rectTmp.left < rect.left) {
                    if (i3 >= end) {
                        float[] fArr = tmpRadii;
                        left = left2;
                        float f = 0.7f * radius * videoPlayerSeekBar.timestampsAppearing;
                        fArr[7] = f;
                        fArr[6] = f;
                        fArr[1] = f;
                        fArr[0] = f;
                        ...
                        float f2 = videoPlayerSeekBar.timestampsAppearing * 0.7f * radius;
                        fArr2[5] = f2;
                        fArr2[4] = f2;
                        fArr2[3] = f2;
                        fArr2[2] = f2;
                ...
                fArr3[c] = radius;
                fArr3[0] = radius;
                float f3 = 0.7f * radius * videoPlayerSeekBar.timestampsAppearing;
                fArr3[5] = f3;
                fArr3[4] = f3;
                fArr3[3] = f3;
                fArr3[2] = f3;
