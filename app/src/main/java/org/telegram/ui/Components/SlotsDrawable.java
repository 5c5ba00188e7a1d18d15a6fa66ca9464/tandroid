...
public class SlotsDrawable extends RLottieDrawable {
    ...
    */
    public /* synthetic */ void m3070lambda$setDiceNumber$10$orgtelegramuiComponentsSlotsDrawable(final TLRPC.TL_messages_stickerSet stickerSet, final int account, final MessageObject currentMessageObject, final ChatMessageCell messageCell, final boolean instant) {
        int num;
        String json;
        if (this.destroyAfterLoading) {
            AndroidUtilities.runOnUIThread(new Runnable() { // from class: org.telegram.ui.Components.SlotsDrawable$$ExternalSyntheticLambda6
                @Override // java.lang.Runnable
                public final void run() {
                    SlotsDrawable.this.m3071lambda$setDiceNumber$6$orgtelegramuiComponentsSlotsDrawable();
                }
            });
            return;
        }
        ...
        while (true) {
            ...
            if (a >= jArr.length + 2) {
                ...
                if (jArr[a] != 0) {
                    ...
                    if (!TextUtils.isEmpty(json)) {
                        ...
                        AndroidUtilities.runOnUIThread(new Runnable() { // from class: org.telegram.ui.Components.SlotsDrawable$$ExternalSyntheticLambda2
                            @Override // java.lang.Runnable
                            public final void run() {
                                SlotsDrawable.lambda$setDiceNumber$7(TLRPC.Document.this, account, currentMessageObject, messageCell, stickerSet);
                            }
                        });
                    } else if (a <= 2) {
                        this.secondNativePtrs[a] = createWithJson(json, "dice", this.metaData, null);
                        this.secondFrameCounts[a] = this.metaData[0];
                    } else {
                        char c = 4;
                        this.nativePtrs[a == 3 ? (char) 0 : (char) 4] = createWithJson(json, "dice", this.metaData, null);
                ...
                final TLRPC.Document document2 = stickerSet.documents.get(num);
                File path2 = FileLoader.getInstance(UserConfig.selectedAccount).getPathToAttach(document2, true);
                json = readRes(path2, 0);
                if (!TextUtils.isEmpty(json)) {
                }
                a++;
            }
        ...
        if (loading) {
            AndroidUtilities.runOnUIThread(new Runnable() { // from class: org.telegram.ui.Components.SlotsDrawable$$ExternalSyntheticLambda7
                @Override // java.lang.Runnable
                public final void run() {
                    SlotsDrawable.this.m3072lambda$setDiceNumber$8$orgtelegramuiComponentsSlotsDrawable();
                }
            });
        } else {
            AndroidUtilities.runOnUIThread(new Runnable() { // from class: org.telegram.ui.Components.SlotsDrawable$$ExternalSyntheticLambda1
                @Override // java.lang.Runnable
                public final void run() {
                    SlotsDrawable.this.m3073lambda$setDiceNumber$9$orgtelegramuiComponentsSlotsDrawable(instant, account, messageCell);
                }
            });
        }
    }
