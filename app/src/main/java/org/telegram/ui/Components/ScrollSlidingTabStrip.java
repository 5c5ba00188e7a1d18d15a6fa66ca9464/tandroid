...
public class ScrollSlidingTabStrip extends HorizontalScrollView {
    ...
    */
    protected void dispatchDraw(Canvas canvas) {
        float dif = this.stickerTabWidth - this.stickerTabExpandedWidth;
        float offset = this.expandOffset * (1.0f - this.expandProgress);
        for (int i = 0; i < this.tabsContainer.getChildCount(); i++) {
            if (this.tabsContainer.getChildAt(i) instanceof StickerTabView) {
                ...
                if (this.animateToExpanded) {
                    stickerTabView.setTranslationX((i * dif * (1.0f - this.expandProgress)) + offset + stickerTabView.dragOffset);
                } else {
        ...
        if (!isInEditMode() && this.tabCount != 0) {
            ...
            if (this.indicatorHeight >= 0) {
                ...
                if (this.animateToExpanded) {
                    float f = this.stickerTabWidth;
                    width = f + ((this.stickerTabExpandedWidth - f) * this.expandProgress);
                }
                if (this.animateFromPosition) {
                    long newTime = SystemClock.elapsedRealtime();
                    long dt = newTime - this.lastAnimationTime;
                    this.lastAnimationTime = newTime;
                    float f2 = this.positionAnimationProgress + (((float) dt) / 150.0f);
                    ...
                    lineLeft = f3 + ((lineLeft - f3) * CubicBezierInterpolator.EASE_OUT_QUINT.getInterpolation(this.positionAnimationProgress));
                    invalidate();
                }
                View view = this.draggingView;
                if (view != null) {
                    float f4 = this.draggingViewIndicatorOutProgress;
                    if (f4 != 1.0f) {
                        float f5 = f4 + 0.10666667f;
                        this.draggingViewIndicatorOutProgress = f5;
                        if (f5 > 1.0f) {
                            this.draggingViewIndicatorOutProgress = 1.0f;
                        } else {
                            invalidate();
                        }
                        switch (AnonymousClass7.$SwitchMap$org$telegram$ui$Components$ScrollSlidingTabStrip$Type[this.type.ordinal()]) {
                            case 1:
                                int i2 = this.indicatorHeight;
                                if (i2 == 0) {
                                    this.indicatorDrawable.setBounds((int) lineLeft, 0, (int) (lineLeft + width), (int) height);
                                    break;
                                } else {
                                    this.indicatorDrawable.setBounds((int) lineLeft, (int) (height - i2), (int) (lineLeft + width), (int) height);
                                    break;
                                }
                            case 2:
                                float yOffset = AndroidUtilities.dp(3.0f) * this.draggingViewIndicatorOutProgress;
                                this.indicatorDrawable.setBounds(((int) lineLeft) + AndroidUtilities.dp(6.0f), (int) ((height - AndroidUtilities.dp(3.0f)) + yOffset), (int) ((lineLeft + width) - AndroidUtilities.dp(6.0f)), (int) (height + yOffset));
                                break;
                        }
                ...
                if (view == null) {
                    float f6 = this.draggingViewIndicatorOutProgress;
                    if (f6 != 0.0f) {
                        float f7 = f6 - 0.10666667f;
                        this.draggingViewIndicatorOutProgress = f7;
                        if (f7 < 0.0f) {
