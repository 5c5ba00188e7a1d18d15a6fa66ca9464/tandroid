        ...
        */
        protected void onDraw(Canvas canvas) {
            ...
            if (z) {
                ...
                if (f < 1.0f) {
                    float f2 = f + (((float) dt) / ANIM_DURATION);
                    this.progress = f2;
                    if (f2 < 1.0f) {
                    ...
                    interpolatedProgress = this.interpolator.getInterpolation(this.progress);
                    this.backgroundDrawable.setAlpha((int) (180.0f * interpolatedProgress));
                    this.backgroundDrawable.draw(canvas);
                    if (interpolatedProgress < 1.0f) {
                        canvas.scale(AndroidUtilities.lerp(0.95f, 1.0f, interpolatedProgress), AndroidUtilities.lerp(0.95f, 1.0f, interpolatedProgress), this.imageReceiver.getCenterX(), this.imageReceiver.getCenterY());
                    }
                    statusBarHeight = Build.VERSION.SDK_INT < 21 ? AndroidUtilities.statusBarHeight : 0;
                    int navBarHeight = Build.VERSION.SDK_INT < 21 ? this.insets.getStableInsetBottom() : 0;
                    int sheetHeight = (this.menuItems.length * AndroidUtilities.dp(48.0f)) + AndroidUtilities.dp(16.0f);
                    float maxBottom = getHeight() - ((navBarHeight + sheetHeight) + AndroidUtilities.dp(16.0f));
                    translationY = Math.min(0.0f, maxBottom - this.imageReceiver.getImageY2());
                    if (this.imageReceiver.getImageY() + translationY >= statusBarHeight) {
                        canvas.translate(0.0f, this.moveProgress * AndroidUtilities.dp(16.0f));
                    } else {
                        canvas.translate(0.0f, (this.moveProgress * AndroidUtilities.dp(16.0f)) + translationY);
                    }
                    this.imageReceiver.setAlpha(interpolatedProgress);
                    ...
                    if (!this.showProgress) {
                        ...
                        if ((drawable instanceof AnimatedFileDrawable) && ((AnimatedFileDrawable) drawable).getDurationMs() > 0) {
                            ...
                            if (valueAnimator2 == null) {
                                ...
                                this.progressHideAnimator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() { // from class: org.telegram.ui.AvatarPreviewer$Layout$$ExternalSyntheticLambda0
                                    @Override // android.animation.ValueAnimator.AnimatorUpdateListener
                                    public final void onAnimationUpdate(ValueAnimator valueAnimator3) {
                                        AvatarPreviewer.Layout.this.m1565lambda$onDraw$3$orgtelegramuiAvatarPreviewer$Layout(valueAnimator3);
                                    }
                                });
                            ...
                            ofFloat2.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() { // from class: org.telegram.ui.AvatarPreviewer$Layout$$ExternalSyntheticLambda1
                                @Override // android.animation.ValueAnimator.AnimatorUpdateListener
                                public final void onAnimationUpdate(ValueAnimator valueAnimator3) {
                                    AvatarPreviewer.Layout.this.m1566lambda$onDraw$4$orgtelegramuiAvatarPreviewer$Layout(valueAnimator3);
                                }
                            });
                    ...
                    if (valueAnimator == null) {
                        ...
                        this.arrowDrawable.setAlpha((int) (255.0f * interpolatedProgress));
                    }
            ...
            if (!z) {
                ...
                if (f3 > 0.0f) {
                    float f4 = f3 - (((float) dt) / ANIM_DURATION);
                    this.progress = f4;
                    if (f4 > 0.0f) {
                        postInvalidateOnAnimation();
                    } else {
            ...
            int sheetHeight2 = (this.menuItems.length * AndroidUtilities.dp(48.0f)) + AndroidUtilities.dp(16.0f);
            float maxBottom2 = getHeight() - ((navBarHeight + sheetHeight2) + AndroidUtilities.dp(16.0f));
            translationY = Math.min(0.0f, maxBottom2 - this.imageReceiver.getImageY2());
            if (this.imageReceiver.getImageY() + translationY >= statusBarHeight) {
            }
            this.imageReceiver.setAlpha(interpolatedProgress);
            this.imageReceiver.draw(canvas);
