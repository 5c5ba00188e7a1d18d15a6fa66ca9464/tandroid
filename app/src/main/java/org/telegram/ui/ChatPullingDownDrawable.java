    ...
    public void drawBottomPanel(Canvas canvas, int top, int bottom, int width) {
        ...
        if (z) {
            ...
            if (f != 1.0f) {
                ...
                int oldAlphaText = this.textPaint2.getAlpha();
                composeBackgroundPaint.setAlpha((int) (oldAlpha * this.progressToBottomPanel));
                canvas.drawRect(0.0f, top, width, bottom, composeBackgroundPaint);
                if (this.layout1 != null) {
                    float f3 = this.swipeToReleaseProgress;
                    if (f3 < 1.0f) {
                        this.textPaint2.setAlpha((int) (oldAlphaText * (1.0f - f3) * this.progressToBottomPanel));
                        float y = (top + (((bottom - top) - this.layout1.getHeight()) / 2.0f)) - (AndroidUtilities.dp(10.0f) * this.swipeToReleaseProgress);
                        canvas.save();
                        canvas.translate((this.lastWidth - this.layout1Width) / 2.0f, y);
                ...
                if (this.layout2 != null) {
                    float f4 = this.swipeToReleaseProgress;
                    if (f4 > 0.0f) {
                        this.textPaint2.setAlpha((int) (oldAlphaText * f4 * this.progressToBottomPanel));
                        float y2 = top + (((bottom - top) - this.layout2.getHeight()) / 2.0f) + (AndroidUtilities.dp(10.0f) * (1.0f - this.swipeToReleaseProgress));
                        canvas.save();
                        canvas.translate((this.lastWidth - this.layout2Width) / 2.0f, y2);
                        this.layout2.draw(canvas);
                        canvas.restore();
                    }
                }
                this.textPaint2.setAlpha(oldAlphaText);
                composeBackgroundPaint.setAlpha(oldAlpha);
            }
        }
        if (!z) {
            float f5 = this.progressToBottomPanel;
            if (f5 != 0.0f) {
                float f6 = f5 - 0.10666667f;
                this.progressToBottomPanel = f6;
                if (f6 < 0.0f) {
        ...
        this.textPaint2.setColor(getThemedColor(Theme.key_chat_messagePanelHint));
        Paint composeBackgroundPaint2 = getThemedPaint(Theme.key_paint_chatComposeBackground);
        int oldAlpha2 = composeBackgroundPaint2.getAlpha();
        int oldAlphaText2 = this.textPaint2.getAlpha();
        composeBackgroundPaint2.setAlpha((int) (oldAlpha2 * this.progressToBottomPanel));
        canvas.drawRect(0.0f, top, width, bottom, composeBackgroundPaint2);
        if (this.layout1 != null) {
        }
        if (this.layout2 != null) {
        }
        this.textPaint2.setAlpha(oldAlphaText2);
        composeBackgroundPaint2.setAlpha(oldAlpha2);
    }
