...
public class TextMessageEnterTransition implements MessageEnterTransitionContainer.Transition {
    ...
    */
    public TextMessageEnterTransition(final ChatMessageCell messageView, final ChatActivity chatActivity, RecyclerListView listView, final MessageEnterTransitionContainer container, Theme.ResourcesProvider resourcesProvider) {
        ...
        float minX;
        int i2;
        StaticLayout messageTextLayout;
        int linesOffset2;
        int rtlLinesCount;
        int n;
        int n2;
        int n3;
        StaticLayout messageTextLayout2;
        boolean z;
        Theme.MessageDrawable drawable;
        CharSequence text2;
        int n4;
        int normalLinesCount;
        int i3;
        int linesOffset3;
        ...
        if (messageView.getMessageObject().textLayoutBlocks.size() <= 1 && !messageView.getMessageObject().textLayoutBlocks.isEmpty()) {
            if (messageView.getMessageObject().textLayoutBlocks.get(0).textLayout.getLineCount() > 10) {
                return;
            }
            ...
            if (chatActivityEnterView != null && chatActivityEnterView.getEditField() != null && chatActivityEnterView.getEditField().getLayout() != null) {
                ...
                CharSequence text3 = messageView.getMessageObject().messageText;
                this.crossfade = false;
                int layoutH = chatActivityEnterView.getEditField().getLayout().getHeight();
                TextPaint textPaint2 = Theme.chat_msgTextPaint;
                int emojiSize2 = AndroidUtilities.dp(20.0f);
                if (messageView.getMessageObject().getEmojiOnlyCount() != 0) {
                    if (messageView.getMessageObject().getEmojiOnlyCount() == 1) {
                        TextPaint textPaint3 = Theme.chat_msgTextPaintOneEmoji;
                        emojiSize2 = AndroidUtilities.dp(32.0f);
                        textPaint = textPaint3;
                    } else if (messageView.getMessageObject().getEmojiOnlyCount() == 2) {
                        TextPaint textPaint4 = Theme.chat_msgTextPaintTwoEmoji;
                        emojiSize2 = AndroidUtilities.dp(28.0f);
                        textPaint = textPaint4;
                    } else if (messageView.getMessageObject().getEmojiOnlyCount() == 3) {
                        TextPaint textPaint5 = Theme.chat_msgTextPaintThreeEmoji;
                        emojiSize2 = AndroidUtilities.dp(24.0f);
                        textPaint = textPaint5;
                    }
                    if (text3 instanceof Spannable) {
                        containsSpans2 = false;
                        linesOffset = 0;
                    } else {
                        Spannable spannable = (Spannable) text3;
                        containsSpans2 = false;
                        linesOffset = 0;
                        Object[] objects = spannable.getSpans(0, text3.length(), Object.class);
                    ...
                    containsSpans = containsSpans2;
                    if (editText.length() == text3.length() || containsSpans) {
                        this.crossfade = true;
                        String str = editText.toString();
                        String trimmedStr = str.trim();
                        i3 = str.indexOf(trimmedStr);
                        if (i3 <= 0) {
                            linesOffset3 = chatActivityEnterView.getEditField().getLayout().getLineTop(chatActivityEnterView.getEditField().getLayout().getLineForOffset(i3));
                            layoutH = chatActivityEnterView.getEditField().getLayout().getLineBottom(chatActivityEnterView.getEditField().getLayout().getLineForOffset(i3 + trimmedStr.length())) - linesOffset3;
                        } else {
                            linesOffset3 = linesOffset;
                        }
                        i = linesOffset3;
                        text = Emoji.replaceEmoji(trimmedStr, textPaint.getFontMetricsInt(), emojiSize2, false);
                    } else {
                        i = linesOffset;
                        text = text3;
                    }
                    this.scaleFrom = chatActivityEnterView.getEditField().getTextSize() / textPaint.getTextSize();
                    int n5 = chatActivityEnterView.getEditField().getLayout().getLineCount();
                    int width = (int) (chatActivityEnterView.getEditField().getLayout().getWidth() / this.scaleFrom);
                    if (Build.VERSION.SDK_INT < 24) {
                        emojiSize = emojiSize2;
                        this.layout = StaticLayout.Builder.obtain(text, 0, text.length(), textPaint, width).setBreakStrategy(1).setHyphenationFrequency(0).setAlignment(Layout.Alignment.ALIGN_NORMAL).build();
                    } else {
                        emojiSize = emojiSize2;
                        this.layout = new StaticLayout(text, textPaint, width, Layout.Alignment.ALIGN_NORMAL, 1.0f, 0.0f, false);
                    }
                    ...
                    this.fromStartY = ((textViewY + AndroidUtilities.dp(10.0f)) - chatActivityEnterView.getEditField().getScrollY()) + i;
                    this.toXOffset = 0.0f;
                    minX = Float.MAX_VALUE;
                    for (i2 = 0; i2 < this.layout.getLineCount(); i2++) {
                        float begin = this.layout.getLineLeft(i2);
                        if (begin < minX) {
                            minX = begin;
                        }
                    }
                    ...
                    this.scaleY = layoutH / (this.layout.getHeight() * this.scaleFrom);
                    this.drawableFromTop = textViewY + AndroidUtilities.dp(4.0f);
                    if (this.enterView.isTopViewVisible()) {
                        this.drawableFromTop -= AndroidUtilities.dp(12.0f);
                    }
                    this.drawableFromBottom = textViewY + chatActivityEnterView.getEditField().getMeasuredHeight();
                    ...
                    messageTextLayout = textLayoutBlock.textLayout;
                    int rtlLinesCount2 = 0;
                    int normalLinesCount2 = 0;
                    ...
                    if (messageTextLayout.getLineCount() == this.layout.getLineCount()) {
                        this.crossfade = true;
                        rtlLinesCount = 0;
                        n2 = n5;
                        n = 0;
                    } else {
                        int n6 = messageTextLayout.getLineCount();
                        int i4 = 0;
                        while (true) {
                            if (i4 >= n6) {
                                n4 = n6;
                                normalLinesCount = normalLinesCount2;
                                break;
                            }
                            ...
                            n4 = n6;
                            if (messageTextLayout.getLineEnd(i4) != this.layout.getLineEnd(i4)) {
                                this.crossfade = true;
                                normalLinesCount = normalLinesCount2;
                                break;
                            }
                        ...
                        n = normalLinesCount;
                        rtlLinesCount = rtlLinesCount2;
                        n2 = n4;
                    }
                    float minX2 = Float.MAX_VALUE;
                    if (!this.crossfade || rtlLinesCount <= 0 || n <= 0) {
                        messageTextLayout2 = messageTextLayout;
                        n3 = n2;
                    } else {
                        ...
                        while (i5 < n2) {
                            ...
                            if (isRtlLine(this.layout, i5)) {
                                text2 = text;
                                normalText.setSpan(new EmptyStubSpan(), this.layout.getLineStart(i5), this.layout.getLineEnd(i5), 0);
                                float begin2 = this.layout.getLineLeft(i5);
                                if (begin2 < minX2) {
                                    minX2 = begin2;
                                }
                            } else {
                                text2 = text;
                                rtlText.setSpan(new EmptyStubSpan(), this.layout.getLineStart(i5), this.layout.getLineEnd(i5), 0);
                            }
                            i5++;
                            n2 = n7;
                            text = text2;
                        ...
                        n3 = n2;
                        if (Build.VERSION.SDK_INT >= 24) {
                            this.layout = StaticLayout.Builder.obtain(normalText, 0, normalText.length(), textPaint, width).setBreakStrategy(1).setHyphenationFrequency(0).setAlignment(Layout.Alignment.ALIGN_NORMAL).build();
                            this.rtlLayout = StaticLayout.Builder.obtain(rtlText, 0, rtlText.length(), textPaint, width).setBreakStrategy(1).setHyphenationFrequency(0).setAlignment(Layout.Alignment.ALIGN_NORMAL).build();
                        } else {
                            TextPaint textPaint6 = textPaint;
                            this.layout = new StaticLayout(normalText, textPaint6, width, Layout.Alignment.ALIGN_NORMAL, 1.0f, 0.0f, false);
                            this.rtlLayout = new StaticLayout(rtlText, textPaint6, width, Layout.Alignment.ALIGN_NORMAL, 1.0f, 0.0f, false);
                        }
                    }
                    ...
                    if (this.drawBitmaps) {
                        ...
                        Canvas bitmapCanvas = new Canvas(this.textLayoutBitmap);
                        this.layout.draw(bitmapCanvas);
                        StaticLayout staticLayout = this.rtlLayout;
                        if (staticLayout != null) {
                            this.textLayoutBitmapRtl = Bitmap.createBitmap(staticLayout.getWidth(), this.rtlLayout.getHeight(), Bitmap.Config.ARGB_8888);
                            Canvas bitmapCanvas2 = new Canvas(this.textLayoutBitmapRtl);
                            this.rtlLayout.draw(bitmapCanvas2);
                        }
                        if (this.crossfade) {
                    ...
                    z = messageView.getMessageObject().getReplyMsgId() == 0 && messageView.replyNameLayout != null;
                    this.hasReply = z;
                    if (z) {
                        ...
                        SimpleTextView replyNameTextView2 = chatActivity.getReplyObjectTextView();
                        this.replyFromObjectStartY = replyNameTextView2.getY() + ((View) replyNameTextView2.getParent().getParent()).getY() + ((View) replyNameTextView2.getParent().getParent().getParent()).getY();
                    ...
                    if (messageView.replyNameLayout != null && messageView.replyNameLayout.getText().length() > 1 && messageView.replyNameLayout.getPrimaryHorizontal(0) != 0.0f) {
                        this.replyNameDx = messageView.replyNameLayout.getWidth() - messageView.replyNameLayout.getLineWidth(0);
                    }
                    if (messageView.replyTextLayout != null && messageView.replyTextLayout.getText().length() >= 1 && messageView.replyTextLayout.getPrimaryHorizontal(0) != 0.0f) {
                    ...
                    ofFloat.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() { // from class: org.telegram.ui.TextMessageEnterTransition$$ExternalSyntheticLambda0
                        ...
                        public final void onAnimationUpdate(ValueAnimator valueAnimator) {
                            TextMessageEnterTransition.this.m4614lambda$new$0$orgtelegramuiTextMessageEnterTransition(chatActivityEnterView, container, valueAnimator);
                        }
                    });
                    ...
                    if (SharedConfig.getDevicePerformanceClass() != 2 && (drawable = messageView.getCurrentBackgroundDrawable(true)) != null) {
                        this.fromMessageDrawable = drawable.getTransitionDrawable(getThemedColor(Theme.key_chat_messagePanelBackground));
                        return;
                    }
                }
                ...
                containsSpans = containsSpans2;
                if (editText.length() == text3.length()) {
                }
                this.crossfade = true;
                String str2 = editText.toString();
                String trimmedStr2 = str2.trim();
                i3 = str2.indexOf(trimmedStr2);
                if (i3 <= 0) {
                }
                i = linesOffset3;
                text = Emoji.replaceEmoji(trimmedStr2, textPaint.getFontMetricsInt(), emojiSize2, false);
                this.scaleFrom = chatActivityEnterView.getEditField().getTextSize() / textPaint.getTextSize();
                int n52 = chatActivityEnterView.getEditField().getLayout().getLineCount();
                int width2 = (int) (chatActivityEnterView.getEditField().getLayout().getWidth() / this.scaleFrom);
                if (Build.VERSION.SDK_INT < 24) {
                }
                float textViewY2 = chatActivityEnterView.getY() + chatActivityEnterView.getEditField().getY() + ((View) chatActivityEnterView.getEditField().getParent()).getY() + ((View) chatActivityEnterView.getEditField().getParent().getParent()).getY();
                this.fromStartX = chatActivityEnterView.getX() + chatActivityEnterView.getEditField().getX() + ((View) chatActivityEnterView.getEditField().getParent()).getX() + ((View) chatActivityEnterView.getEditField().getParent().getParent()).getX();
                this.fromStartY = ((textViewY2 + AndroidUtilities.dp(10.0f)) - chatActivityEnterView.getEditField().getScrollY()) + i;
                this.toXOffset = 0.0f;
                minX = Float.MAX_VALUE;
                ...
                this.scaleY = layoutH / (this.layout.getHeight() * this.scaleFrom);
                this.drawableFromTop = textViewY2 + AndroidUtilities.dp(4.0f);
                if (this.enterView.isTopViewVisible()) {
                }
                this.drawableFromBottom = textViewY2 + chatActivityEnterView.getEditField().getMeasuredHeight();
                MessageObject.TextLayoutBlock textLayoutBlock2 = messageView.getMessageObject().textLayoutBlocks.get(0);
                this.textLayoutBlock = textLayoutBlock2;
                messageTextLayout = textLayoutBlock2.textLayout;
                int rtlLinesCount22 = 0;
                int normalLinesCount22 = 0;
                if (Math.abs(ColorUtils.calculateLuminance(getThemedColor(Theme.key_chat_messageTextOut)) - ColorUtils.calculateLuminance(getThemedColor(Theme.key_chat_messagePanelText))) > 0.20000000298023224d) {
                }
                this.fromColor = getThemedColor(Theme.key_chat_messagePanelText);
                this.toColor = getThemedColor(Theme.key_chat_messageTextOut);
                ...
                if (messageView.getMessageObject().getReplyMsgId() == 0) {
                }
                this.hasReply = z;
                if (z) {
                }
