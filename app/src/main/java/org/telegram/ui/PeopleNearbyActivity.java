    ...
    */
    public void didReceivedNotification(int id, int account, Object... args) {
        ...
        if (id == NotificationCenter.newLocationAvailable) {
            ...
            diffCallback.saveCurrentState();
            int b = 0;
            int N23 = update.peers.size();
            while (b < N23) {
                TLRPC.PeerLocated object2 = update.peers.get(b);
                if (!(object2 instanceof TLRPC.TL_peerLocated)) {
                    N2 = N23;
                } else {
                    TLRPC.TL_peerLocated peerLocated = (TLRPC.TL_peerLocated) object2;
                    boolean found = false;
                    if (peerLocated.peer instanceof TLRPC.TL_peerUser) {
                    ...
                    while (a < N) {
                        ...
                        if (old.peer.chat_id != 0) {
                        }
                        if (old.peer.channel_id != 0) {
                            if (old.peer.channel_id != peerLocated.peer.channel_id) {
                            }
            ...
            checkForExpiredLocations(true);
            updateRows(diffCallback);
        } else if (id != NotificationCenter.needDeleteDialog || this.fragmentView == null || this.isPaused) {
        } else {
            ...
            Runnable deleteRunnable = new Runnable() { // from class: org.telegram.ui.PeopleNearbyActivity$$ExternalSyntheticLambda6
                @Override // java.lang.Runnable
                public final void run() {
                    PeopleNearbyActivity.this.m4167xa0217175(chat, dialogId, revoke);
                }
            };
            UndoView undoView = this.undoView;
            if (undoView != null) {
                undoView.showWithAction(dialogId, 1, deleteRunnable);
            } else {
                deleteRunnable.run();
            }
        }
