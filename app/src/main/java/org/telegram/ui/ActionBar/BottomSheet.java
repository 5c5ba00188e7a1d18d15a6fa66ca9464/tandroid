    ...
    public class ContainerView extends FrameLayout implements NestedScrollingParent {
        ...
        */
        protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
            ...
            int oldKeyboardHeight = this.keyboardHeight;
            boolean z = false;
            if (this.rect.bottom != 0 && this.rect.top != 0) {
                int usableViewHeight = (int) ((rootView.getHeight() - (this.rect.top != 0 ? AndroidUtilities.statusBarHeight * (1.0f - BottomSheet.this.hideSystemVerticalInsetsProgress) : 0.0f)) - (AndroidUtilities.getViewInset(rootView) * (1.0f - BottomSheet.this.hideSystemVerticalInsetsProgress)));
            ...
            if (oldKeyboardHeight != i2) {
                this.keyboardChanged = true;
            }
            BottomSheet.this.keyboardVisible = i2 > AndroidUtilities.dp(20.0f);
            if (BottomSheet.this.lastInsets != null && Build.VERSION.SDK_INT >= 21) {
                ...
                if (BottomSheet.this.keyboardVisible && this.rect.bottom != 0 && this.rect.top != 0) {
                    BottomSheet.access$720(BottomSheet.this, this.keyboardHeight);
                }
                if (!BottomSheet.this.drawNavigationBar) {
                    int containerHeight2 = height2 - BottomSheet.this.getBottomInset();
                    containerHeight = containerHeight2;
                    setMeasuredDimension(width2, containerHeight);
                    if (BottomSheet.this.lastInsets == null && Build.VERSION.SDK_INT >= 21) {
                        int inset = (int) (BottomSheet.this.lastInsets.getSystemWindowInsetBottom() * (1.0f - BottomSheet.this.hideSystemVerticalInsetsProgress));
                        if (Build.VERSION.SDK_INT >= 29) {
                            inset += BottomSheet.this.getAdditionalMandatoryOffsets();
                        }
                        height = height2 - inset;
                    } else {
                        height = height2;
                    }
                    if (BottomSheet.this.lastInsets == null && Build.VERSION.SDK_INT >= 21) {
                        width = width2 - (BottomSheet.this.getRightInset() + BottomSheet.this.getLeftInset());
                    } else {
                        width = width2;
                    }
                    ...
                    bottomSheet5.isPortrait = z;
                    if (BottomSheet.this.containerView != null) {
                        if (BottomSheet.this.fullWidth) {
                            BottomSheet.this.containerView.measure(View.MeasureSpec.makeMeasureSpec((BottomSheet.this.backgroundPaddingLeft * 2) + width, C.BUFFER_FLAG_ENCRYPTED), View.MeasureSpec.makeMeasureSpec(height, Integer.MIN_VALUE));
                        } else {
                            if (AndroidUtilities.isTablet()) {
                                widthSpec = View.MeasureSpec.makeMeasureSpec(((int) (Math.min(AndroidUtilities.displaySize.x, AndroidUtilities.displaySize.y) * 0.8f)) + (BottomSheet.this.backgroundPaddingLeft * 2), C.BUFFER_FLAG_ENCRYPTED);
                            } else {
                                BottomSheet bottomSheet6 = BottomSheet.this;
                                widthSpec = View.MeasureSpec.makeMeasureSpec(bottomSheet6.getBottomSheetWidth(bottomSheet6.isPortrait, width, height) + (BottomSheet.this.backgroundPaddingLeft * 2), C.BUFFER_FLAG_ENCRYPTED);
                            }
