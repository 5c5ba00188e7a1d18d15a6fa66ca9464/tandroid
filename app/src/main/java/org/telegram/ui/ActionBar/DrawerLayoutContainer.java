        ...
        if ((this.allowOpenDrawerBySwipe || this.drawerOpened) && this.allowOpenDrawer && this.parentActionBarLayout.fragmentsStack.size() == 1) {
            if (ev == null || ((ev.getAction() != 0 && ev.getAction() != 2) || this.startedTracking || this.maybeStartTracking)) {
                if (ev != null && ev.getAction() == 2 && ev.getPointerId(0) == this.startedTrackingPointerId) {
                    ...
                    float dx = (int) (ev.getX() - this.startedTrackingX);
                    float dy = Math.abs(((int) ev.getY()) - this.startedTrackingY);
                    this.velocityTracker.addMovement(ev);
                    if (this.maybeStartTracking && !this.startedTracking && ((dx > 0.0f && dx / 3.0f > Math.abs(dy) && Math.abs(dx) >= AndroidUtilities.getPixelsInCM(0.2f, true)) || (this.drawerOpened && dx < 0.0f && Math.abs(dx) >= Math.abs(dy) && Math.abs(dx) >= AndroidUtilities.getPixelsInCM(0.4f, true)))) {
                    ...
                    boolean backAnimation = (this.drawerPosition < ((float) this.drawerLayout.getMeasuredWidth()) / 2.0f && (velX < 3500.0f || Math.abs(velX) < Math.abs(velY))) || (velX < 0.0f && Math.abs(velX) >= 3500.0f);
                    if (!backAnimation) {
