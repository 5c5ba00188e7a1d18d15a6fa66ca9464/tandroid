...
public class GroupCallActivity extends BottomSheet implements NotificationCenter.NotificationCenterDelegate, VoIPService.StateListener {
    ...
    */
    public void applyCallParticipantUpdates(boolean animated) {
        int count;
        int minPosition;
        int a;
        int i;
        boolean hasVideoLocal;
        boolean foundAnotherVideoParticipant;
        RecyclerView.ViewHolder holder;
        Exception e;
        ...
        long self = MessageObject.getPeerId(this.call.selfPeer);
        long dummyPeer = MessageObject.getPeerId(this.selfPeer);
        if (self != dummyPeer && this.call.participants.get(self) != null) {
            this.selfPeer = this.call.selfPeer;
        }
        int count2 = this.listView.getChildCount();
        View minChild = null;
        int minPosition2 = 0;
        int minTop2 = Integer.MAX_VALUE;
        for (int a2 = 0; a2 < count2; a2++) {
            View child = this.listView.getChildAt(a2);
            RecyclerView.ViewHolder holder2 = this.listView.findContainingViewHolder(child);
            if (holder2 != null && holder2.getAdapterPosition() != -1 && holder2.getLayoutPosition() != -1 && (minChild == null || child.getTop() < minTop2)) {
                int minPosition3 = holder2.getLayoutPosition();
                minChild = child;
                minPosition2 = minPosition3;
                minTop2 = child.getTop();
            }
        }
        updateVideoParticipantList();
        if (this.listView.getItemAnimator() != null && !animated) {
            this.listView.setItemAnimator(null);
        } else if (this.listView.getItemAnimator() == null && animated) {
            this.listView.setItemAnimator(this.itemAnimator);
        }
        try {
            updateCallback = new UpdateCallback(this.listAdapter);
            i2 = this.listAdapter.addMemberRow;
            i3 = this.listAdapter.usersStartRow;
            i4 = this.listAdapter.usersEndRow;
            i5 = this.listAdapter.invitedStartRow;
            i6 = this.listAdapter.invitedEndRow;
            minTop = this.listAdapter.usersVideoGridStartRow;
        ...
        try {
            setOldRows(i2, i3, i4, i5, i6, minTop, this.listAdapter.usersVideoGridEndRow, this.listAdapter.videoGridDividerRow, this.listAdapter.videoNotAvailableRow);
            ...
            boolean autoPinEnabled = this.renderersContainer.autoPinEnabled();
            if (this.renderersContainer.inFullscreenMode) {
                foundAnotherVideoParticipant = false;
            ...
            hasVideoLocal = !this.call.visibleVideoParticipants.isEmpty();
            if (hasVideoLocal == this.hasVideo) {
            }
        }
        this.call.saveActiveDates();
        if (minChild != null) {
            this.layoutManager.scrollToPositionWithOffset(minPosition, minChild.getTop() - this.listView.getPaddingTop());
        }
        this.oldParticipants.clear();
        this.oldParticipants.addAll(this.call.visibleParticipants);
        this.oldVideoParticipants.clear();
        ...
        while (true) {
            ...
            View child2 = this.listView.getChildAt(a);
            if (((child2 instanceof GroupCallUserCell) || (child2 instanceof GroupCallInvitedCell)) && (holder = this.listView.findContainingViewHolder(child2)) != null) {
                if (child2 instanceof GroupCallUserCell) {
                    GroupCallUserCell groupCallUserCell = (GroupCallUserCell) child2;
                    if (holder.getAdapterPosition() != this.listAdapter.getItemCount() - 2) {
                        z2 = true;
                    }
                    groupCallUserCell.setDrawDivider(z2);
                } else {
                    GroupCallInvitedCell groupCallInvitedCell = (GroupCallInvitedCell) child2;
                    if (holder.getAdapterPosition() != this.listAdapter.getItemCount() - 2) {
                        z2 = true;
                    }
        ...
        if (this.renderersContainer.inFullscreenMode && this.renderersContainer.fullscreenParticipant != null && !ChatObject.Call.videoIsActive(this.renderersContainer.fullscreenParticipant.participant, this.renderersContainer.fullscreenParticipant.presentation, this.call)) {
            foundAnotherVideoParticipant = false;
            if (!this.visibleVideoParticipants.isEmpty()) {
        ...
        this.attachedRenderersTmp.clear();
        this.attachedRenderersTmp.addAll(this.attachedRenderers);
        for (i = 0; i < this.attachedRenderersTmp.size(); i++) {
    ...
    private GroupCallActivity(final Context context, final AccountInstance account, ChatObject.Call groupCall, final TLRPC.Chat chat, TLRPC.InputPeer schedulePeer, boolean scheduleHasFewPeers, String scheduledHash) {
        ...
        RecyclerListView recyclerListView = new RecyclerListView(context) { // from class: org.telegram.ui.GroupCallActivity.9
            ...
            */
            public void dispatchDraw(Canvas canvas) {
                boolean visiblePeersChanged;
                float maxBottom;
                float minTop;
                boolean visiblePeersChanged2;
                boolean visiblePeersChanged3;
                float inMaxBottom = 0.0f;
                float inMinTop = Float.MAX_VALUE;
                int i3 = 1;
                boolean animateBackground = GroupCallActivity.this.itemAnimator.outMinTop != Float.MAX_VALUE;
                ...
                boolean visiblePeersChanged4 = false;
                int a = 0;
                int N = getChildCount();
                while (a < N) {
                    ...
                    if (holder == null || holder.getItemViewType() == 3 || holder.getItemViewType() == 4 || holder.getItemViewType() == 5) {
                        visiblePeersChanged2 = visiblePeersChanged4;
                    } else if (holder.getItemViewType() == 6) {
                        visiblePeersChanged2 = visiblePeersChanged4;
                    } else {
                        if (holder.getItemViewType() != i3 || !(holder.itemView instanceof GroupCallUserCell)) {
                            visiblePeersChanged3 = visiblePeersChanged4;
                        } else {
                            GroupCallUserCell userCell = (GroupCallUserCell) holder.itemView;
                            GroupCallActivity.this.visiblePeerIds.append(userCell.getPeerId(), i3);
                            visiblePeersChanged3 = visiblePeersChanged4;
                            if (this.visiblePeerTmp.get(userCell.getPeerId(), 0) == 0) {
                                visiblePeersChanged4 = true;
                                if (!animateBackground) {
                                    if (!GroupCallActivity.this.itemAnimator.removingHolders.contains(holder)) {
                                    ...
                                    inMinTop = Math.min(inMinTop, Math.max(0.0f, child.getY()));
                                }
                                a++;
                                i3 = 1;
                            } else {
                                this.visiblePeerTmp.delete(userCell.getPeerId());
                            }
                        }
                        visiblePeersChanged4 = visiblePeersChanged3;
                        if (!animateBackground) {
                        }
                        a++;
                        i3 = 1;
                    }
                    visiblePeersChanged4 = visiblePeersChanged2;
                    a++;
                    i3 = 1;
                }
                boolean visiblePeersChanged5 = visiblePeersChanged4;
                if (this.visiblePeerTmp.size() <= 0) {
                    visiblePeersChanged = visiblePeersChanged5;
                } else {
                ...
                if (animateBackground) {
                    minTop = (GroupCallActivity.this.itemAnimator.outMinTop * (1.0f - GroupCallActivity.this.itemAnimator.animationProgress)) + (GroupCallActivity.this.itemAnimator.animationProgress * inMinTop);
                    maxBottom = (GroupCallActivity.this.itemAnimator.outMaxBottom * (1.0f - GroupCallActivity.this.itemAnimator.animationProgress)) + (GroupCallActivity.this.itemAnimator.animationProgress * inMaxBottom);
                } else {
                    minTop = inMinTop;
                    maxBottom = inMaxBottom;
                }
                if (inMinTop != Float.MAX_VALUE) {
                    int itemsWidth = AndroidUtilities.isTablet() ? Math.min(AndroidUtilities.dp(420.0f), getMeasuredWidth()) : getMeasuredWidth();
                    int padding = (getMeasuredWidth() - itemsWidth) >> 1;
                    GroupCallActivity.this.rect.set(padding, minTop, getMeasuredWidth() - padding, Math.min(getMeasuredHeight() - getTranslationY(), maxBottom));
    ...
    */
    private void openShareAlert(boolean withMessage, String urlMuted, String urlUnmuted, boolean copy) {
        ...
        String message;
        if (!isRtmpStream()) {
            urlUnmuted2 = urlUnmuted;
        } else {
            urlUnmuted2 = null;
        }
        if (copy) {
            AndroidUtilities.addToClipboard(urlMuted != null ? urlMuted : urlUnmuted2);
        ...
        if (launchActivity != null) {
            BaseFragment fragment = launchActivity.getActionBarLayout().fragmentsStack.get(this.parentActivity.getActionBarLayout().fragmentsStack.size() - 1);
            if (fragment instanceof ChatActivity) {
                boolean keyboardIsOpen2 = ((ChatActivity) fragment).needEnterText();
                this.anyEnterEventSent = true;
                this.enterEventSent = true;
                keyboardIsOpen = keyboardIsOpen2;
                if (urlMuted == null && urlUnmuted2 == null) {
                    urlUnmuted3 = urlMuted;
                    urlMuted2 = null;
                } else {
                    urlMuted2 = urlMuted;
                    urlUnmuted3 = urlUnmuted2;
                }
                if (urlMuted2 != null && withMessage) {
                    if (ChatObject.isChannelOrGiga(this.currentChat)) {
                        message = LocaleController.formatString("VoipChannelInviteText", R.string.VoipChannelInviteText, urlUnmuted3);
                    } else {
                        message = LocaleController.formatString("VoipGroupInviteText", R.string.VoipGroupInviteText, urlUnmuted3);
                    }
                } else {
                    String message2 = urlUnmuted3;
                    message = message2;
                }
                ShareAlert shareAlert = new ShareAlert(getContext(), null, null, message, urlMuted2, false, urlUnmuted3, urlMuted2, false, true) { // from class: org.telegram.ui.GroupCallActivity.46
                    @Override // org.telegram.ui.Components.ShareAlert
                    protected void onSend(LongSparseArray<TLRPC.Dialog> dids, int count) {
                        if (dids.size() == 1) {
                            GroupCallActivity.this.getUndoView().showWithAction(dids.valueAt(0).id, 41, Integer.valueOf(count));
                        } else {
                            GroupCallActivity.this.getUndoView().showWithAction(0L, 41, Integer.valueOf(count), Integer.valueOf(dids.size()), (Runnable) null, (Runnable) null);
                        }
                    }
                };
                ...
                this.shareAlert.setOnDismissListener(new DialogInterface.OnDismissListener() { // from class: org.telegram.ui.GroupCallActivity$$ExternalSyntheticLambda3
                    @Override // android.content.DialogInterface.OnDismissListener
                    public final void onDismiss(DialogInterface dialogInterface) {
                        GroupCallActivity.this.m3498lambda$openShareAlert$42$orgtelegramuiGroupCallActivity(dialogInterface);
                    }
                });
                AndroidUtilities.runOnUIThread(new Runnable() { // from class: org.telegram.ui.GroupCallActivity$$ExternalSyntheticLambda27
                    @Override // java.lang.Runnable
                    public final void run() {
                        GroupCallActivity.this.m3499lambda$openShareAlert$43$orgtelegramuiGroupCallActivity();
                    }
                }, !keyboardIsOpen ? 200L : 0L);
            }
        }
        ...
        String message22 = urlUnmuted3;
        message = message22;
        ShareAlert shareAlert2 = new ShareAlert(getContext(), null, null, message, urlMuted2, false, urlUnmuted3, urlMuted2, false, true) { // from class: org.telegram.ui.GroupCallActivity.46
            @Override // org.telegram.ui.Components.ShareAlert
            protected void onSend(LongSparseArray<TLRPC.Dialog> dids, int count) {
                if (dids.size() == 1) {
                    GroupCallActivity.this.getUndoView().showWithAction(dids.valueAt(0).id, 41, Integer.valueOf(count));
